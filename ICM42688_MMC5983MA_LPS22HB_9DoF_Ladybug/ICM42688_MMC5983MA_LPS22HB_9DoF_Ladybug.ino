...
#define myLed    13
#define CLKOUT    9        // 32 kHz clock output
bool clockIn = true;
...
void setup() {
  Serial.begin(115200);
  while (!Serial) { } //wait for serial monitor to begin
  ...
  pinMode(ICM42688_intPin2, INPUT); // Use INT2 for significant motion or wake on motion
...
//  pinMode(LPS22H_intPin, INPUT);
  ...
  // enable 32.768 kHz output on PA2 (D9)
  stm32l4_gpio_pin_configure(STM32L4_GPIO_PIN_PA2_LSCO, STM32L4_GPIO_MODE_ALTERNATE);
  stm32l4_system_lsco_configure(STM32L4_SYSTEM_LSCO_SOURCE_LSE);
  }
  ...
  if(ICM42688ID == 0x47 && MMC5983ID == 0x30 && LPS22ID == 0xB1) // check if all I2C sensors have acknowledged
  {
   ...
   ICM42688.init(Ascale, Gscale, AODR, GODR, aMode, gMode, clockIn); // configure for basic accel/gyro data output  
...
//    STM32L4.sleep(); // put MCU to sleep/stop while waiting for an interrupt.
